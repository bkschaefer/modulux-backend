...
collectionSchema.pre('save', async function (next) {
  if (this.isNew) {
    try {
      ...
      logger.error(
        `Failed to create entry collection for schema '${this.name}': ${error.message}`,
      )
      throw new ServerError(
        `Failed to create entry collection: ${error.message}`,
      )
    }
  }
  next()
...
collectionSchema.pre('deleteOne', { document: true }, async function (next) {
  try {
    ...
    if (error.code !== 26) {
      // MongoDB error code 26: namespace not found
      logger.error(
        `Failed to drop entry collection for schema '${this.name}': ${error.message}`,
      )
...
async function deleteField(
  fieldName: string,
  parentPath: string,
  EntryModel: mongoose.Model<IEntrySchema>,
) {
...
async function updateEntries(
  ...
  if (queryFields.length < oldFields.length) {
    const fieldToDelete = oldFieldNames.find(
      (name) => !queryFieldNames.includes(name),
    )
    return deleteField(fieldToDelete!, parentPath, EntryModel)
  }
  if (
    queryFields.length === oldFields.length &&
    JSON.stringify(oldFieldNames) !== JSON.stringify(queryFieldNames)
  ) {
    const oldToNewKey = oldFieldNames.reduce(
      (acc, oldFieldName, index) => {
  ...
  queryFields.forEach(async (field, index) => {
    if (field.fieldType === 'CompositeField') {
      ...
      if (oldCompositeFields)
        return await updateEntries(
          oldCompositeFields,
          queryCompositeFields,
          EntryModel,
          field.name,
        )
    }
    if (
      field.fieldType === 'FieldArray' &&
      (field as IFieldArray).field?.fieldType === 'CompositeField'
    ) {
      const queryCompositeFields = (
        (field as IFieldArray).field as ICompositeField
      ).fields
      const oldCompositeFields = (
        (oldFields[index] as IFieldArray).field as ICompositeField
      )?.fields
      if (oldCompositeFields)
        return await updateEntries(
          oldCompositeFields,
...
collectionSchema.pre('updateOne', async function (next) {
  try {
    ...
    const querySchemaFields = JSON.parse(
      (this.getUpdate() as ICollectionSchema).schemaJSON,
    ).fields
