...
export async function getAllCollectionNames() {
  try {
    const allNames = await Collection.find(
      {},
      { name: 1, title: 1, _id: 0 },
    ).exec()
    ...
    throw new ServerError(
      `Failed to load all schema names and titles: ${message}`,
    )
  }
}
...
export async function deleteCollection(collectionName: string): Promise<void> {
  ...
  try {
    ...
    logger.info(
      `Schema '${collectionName}' and its entries deleted successfully.`,
    )
  } catch (err: unknown) {
    const message = err instanceof Error ? err.message : 'Unknown error'
