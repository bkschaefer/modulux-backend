...
export async function getCollectionSchema(collectionName: string): Promise<TSchemaResource> {
  ...
  logger.info(`Schema of the collection '${collectionName}' retrieved successfully.`);
  return JSON.parse(collection.schemaJSON);
}
export async function updateSchemaSettings(collectionName: string, newSettings: Record<string, any>) {
  const collection = await Collection.findOne({ name: collectionName }).exec();
  if (!collection) {
    throw new HttpError400({
      path: "collectionName",
...
export async function updateSchemaFieldSettings(
  ...
  try {
    fieldToUpdate.settings = {
      dataTable: {
        visible: newSettings.dataTable.visible !== undefined ? newSettings.dataTable.visible : fieldToUpdate.settings.dataTable.visible,
        columnWidth: newSettings.dataTable.columnWidth !== undefined ? newSettings.dataTable.columnWidth : fieldToUpdate.settings.dataTable.columnWidth
      }
    };

    schema.fields = schema.fields.map(
      (field: any) => (field.name === fieldName) ? fieldToUpdate : field,
    );
    collection.schemaJSON = JSON.stringify(schema);
    await collection.save();
    logger.info(
